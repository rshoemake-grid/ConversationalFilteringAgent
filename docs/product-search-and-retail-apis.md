...
This document explains **how product lists are produced** and why you see **more than one Google Cloud call** for a single user message in **`convo_commerce`** mode. For a **user-journey-oriented** view (chips, load more, mode comparison), see **[user-flow-and-services.md](user-flow-and-services.md)**. For **HTTP** fields and OpenAPI links, see **[api-reference.md](api-reference.md)**.
